...
public class MP2VoteAndReceipt extends MPAbstractVoteAndReceipt {
	...
	public ElGamalEncryption[] getCanonicalVote(IMPParameters param)
	{
		if (this.canonicalVote == null)
		{
			...
			BigInteger mpG = param.getMP_G();
			ElGamalEncryption[] canonicalVoteAux = new ElGamalEncryption[voteEnc.length];
			...
			for(int i=0; i < voteEnc.length; i++)
			{
				...
				if(subVector[0].equals(BigInteger.ZERO))
				{
					...
					canonicalVoteAux[i] = MP2Util.multiplyByClearTextValue(subVectorEnc[1], 
							canonicalTransformationFactor, p);
				} else {
					canonicalTransformationFactor = subVector[0].modInverse(q);
					canonicalVoteAux[i] = MP2Util.multiplyByClearTextValue(subVectorEnc[0],
							canonicalTransformationFactor, p);
				}

				canonicalVoteAux[i] = MP2Util.multiplyByClearTextValue(canonicalVoteAux[i], TWO, p);
				aux = canonicalVoteAux[i].Y.multiply(param.getMP_GInv());
				canonicalVoteAux[i] = new ElGamalEncryption(canonicalVoteAux[i].X, aux.mod(p));

			}
			this.canonicalVote = canonicalVoteAux;
		}
		return this.canonicalVote;
	}
}
