...
public class MP3PerformanceTestMultiThread implements Runnable{
	...
	public void run()
	{
		...
		try{
			...
			MP3AggregatedElectionVotes electionVoteAggregation = 
				new MP3AggregatedElectionVotes(NUMBER_OF_CANDIDATES, votes, mp3Param, HASH_FUNCTION);
	...
	{	
		...
		startTallyTranslation, endTallyTranslation, start, end;
		...
		println("Total thread time: " + (end - start), mainOutput);
		println("********************************************************", mainOutput);
	}
