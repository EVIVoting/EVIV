...
public class MP3AggregatedElectionVotes {
	 ...
	 * If the vote validation is turned off present all votes are aggregated.
	 **/
	...
	public MP3AggregatedElectionVotes(int numberOfCandidates, MP3VoteAndReceipt[] votes, 
			MP3Parameters mp3Param, String hashFunction, boolean validateVotes) throws NoSuchAlgorithmException
			{
		...
		for(int i=0; i<votes.length; i++)
		{
			...
			try{
				...
				for(int j=0; j < numberOfCandidates; j++)
				{
					...
					if(validateVotes && !CGS97BallotValidity.verifyBallotValidity(cvote.BIT_ENCRYPTION,
		...
		if(!validateVotes)
			System.out.println("The vote verification was turned off: THE VOTES WHERE NOT VALIDATED.");
