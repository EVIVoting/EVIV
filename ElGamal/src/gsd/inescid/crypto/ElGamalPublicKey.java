...
public final class ElGamalPublicKey {
	...
	/**
	 * Exponential encryption of message m in the q-order subgroup of Z*_p 
	 * It uses a random encryption factor in the range [1, q[.
	 * @param m message to encrypt
	 * @return the encrypted message
	 * @throws InvalidParameterException if m >= this.q or m <= 0
	 */
	public final ElGamalEncryption exponentialEncrypt(BigInteger m)
	{
		if (m.compareTo(this.q)>=0 || m.compareTo(BigInteger.ZERO)<=0)
			throw new InvalidParameterException("Message to encrypt must be in Z*_q");
		
		// generate a random encryption factor in the range [1, q[. 
		BigInteger rf = generateRandomFactor();
		BigInteger mExp = this.g.modPow(m, this.p);
		return encryptQOrderMessage(mExp, rf);
	}
	...
	/**
	 * Exponential encryption of message m in the q-order subgroup of Z_p 
	 * It uses a random encryption factor in the range [1, q[.
	 * @param m message to encrypt
	 * @return the encrypted message and the random factor used
	 * @throws InvalidParameterException if m >= this.q or m < 0
	 */
	public final ElGamalVerifiableEncryption exponentialVerifiableEncrypt(BigInteger m)
	{
		if (m.compareTo(this.q)>=0 || m.compareTo(BigInteger.ZERO)<0)
			throw new InvalidParameterException("Message to encrypt must be in Z_q");
		...
		BigInteger rf = generateRandomFactor();
		BigInteger mExp = this.g.modPow(m, this.p);
		return new ElGamalVerifiableEncryption(encryptQOrderMessage(mExp, rf),rf);
	}
